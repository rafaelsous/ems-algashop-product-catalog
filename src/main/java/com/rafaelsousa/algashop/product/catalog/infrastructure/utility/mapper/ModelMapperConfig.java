...
public class ModelMapperConfig {
	private final ApplicationMappingProperty applicationMappingProperty;

	@Autowired
	public ModelMapperConfig(ApplicationMappingProperty applicationMappingProperty) {
		this.applicationMappingProperty = applicationMappingProperty;
	}
    ...
    private void configuration(ModelMapper modelMapper) {
	    ...
	    modelMapper
			    .createTypeMap(Image.class, ImageOutput.class)
			    .addMappings(mapping ->
					    mapping
							    .using(fromFileNameToUrlConverter)
							    .map(Image::getName, ImageOutput::setUrl));

		modelMapper
			.createTypeMap(Product.class, ProductSummaryOutput.class)
			.addMappings(mapping ->
					mapping
						.using(fromStringToShortStringConverter)
